...
# =============================================================================
# status — 현재 접속 정보 다시 보기 (D8, 폰 Termux 에서)
#   Quick Tunnel URL 은 재시작마다 바뀌고 터미널이 스크롤되면 잃기 쉽다.
#   서버를 끄지 않고 현재 주소·자격증명을 로그/상태파일에서 복구해 다시 출력한다.
#   (서버를 새로 켜지 않는다 — 읽기 전용 진단.)
# =============================================================================
# 프로세스 생존 표시 (best-effort; pgrep 없으면 조용히 'unknown')
status_proc() {
  local label="$1" pat="$2"
  if ! command -v pgrep >/dev/null 2>&1; then c_warn "${label}: 확인 불가 (pgrep 없음)"; return; fi
  if pgrep -f "$pat" >/dev/null 2>&1; then c_ok "${label}: 실행 중"; else c_warn "${label}: 미실행"; fi
}

status_main() {
  if [ ! -f "$CRED_FILE" ]; then
    c_err "아직 설치되지 않았습니다 — 먼저 'bash anddev.sh setup' 을 실행하세요."
    exit 1
  fi
  # shellcheck disable=SC1090
  source "$CRED_FILE"
  local host; host="$(tunnel_host_from_log)"

  echo "================ anddev 상태 ================"
  status_proc "cloudflared 터널" "cloudflared tunnel"
  status_proc "sshd"             "sshd -D"
  echo
  if [ -n "$host" ]; then
    cat <<INFO
  호스트   : ${host}
  ...
  접속:  bash anddev.sh connect ${host}
INFO
  else
    c_warn "활성 터널 주소를 찾지 못했습니다."
    c_warn "서버가 꺼져 있을 수 있습니다 → 'bash anddev.sh start' (로그: ${TUNNEL_LOG})"
  fi
  echo "============================================"
}
...
사용 (폰 Termux):
  ...
  bash anddev.sh status     현재 접속 주소·자격증명 다시 보기 (서버는 안 끔)
